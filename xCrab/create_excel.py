        ...
        try:
            ...
            if lease_data and lease_data != 'null':
                ...
                for i, line in enumerate(lines):
                    line = line.strip()
                    # 租金
                    if '租金' in line and '月' in line:
                        # 找到金额
                        for j in range(i, min(len(lines), i+3)):
                            amount_line = lines[j].strip()
                            if '￥' in amount_line or '元' in amount_line:
                                info['租金'] = amount_line
                                break
                    # 押金总额
                    if '押金总额' in line:
                        info['押金总额'] = lines[i+1].strip() if i+1 < len(lines) else ''
                    # 租期起止日
                    if '租期起止日' in line:
                        info['租期起止日'] = lines[i+1].strip() if i+1 < len(lines) else ''
                    # 租客姓名
                    if '租客姓名' in line:
                        info['租客姓名'] = lines[i+1].strip() if i+1 < len(lines) else ''
