...
function connectWebSocket(token) {
    ...
    ws.on('message', (message) => {
        try {
            ...
            if (msg.type === 'auth_success') {
                ...
                const stopUsername = msg.username || 'unknown';
                console.log(`\n[收到停止指令] 用户: ${stopUsername}`);

                // 根据用户名筛选需要停止的会话
                const sessionsToStop = [];
                for (const [sid, state] of sessionChildren) {
                    if (state.username === stopUsername) {
                        sessionsToStop.push(sid);
                    }
                }

                if (sessionsToStop.length > 0) {
                    console.log(`正在停止用户 ${stopUsername} 的 ${sessionsToStop.length} 个会话任务...`);
                    try {
                        if (process.platform === 'win32') {
                            ...
                            for (const sid of sessionsToStop) {
                                const state = sessionChildren.get(sid);
                                if (state) {
                        ...
                        console.log(`已停止用户 ${stopUsername} 的任务`);
                    } catch (e) {
                        console.error('停止任务失败:', e.message);
                    }
                } else {
                    console.log(`用户 ${stopUsername} 没有正在执行的任务`);
                }
    ...
    ws.on('close', () => {
        ...
        reconnectCount++;
        console.log('\n===================================================');
        console.log(`❌ 警告：与云端服务器断开连接，正在尝试重连... (${reconnectCount}/${MAX_RECONNECT_ATTEMPTS})`);
        console.log('===================================================\n');

        // 检查是否超过最大重连次数
        if (reconnectCount >= MAX_RECONNECT_ATTEMPTS) {
            console.log('\n===================================================');
            console.log('❌ 连接已断开，已终止任务！');
            console.log('===================================================\n');
            ...
            if (process.stdin.isTTY) {
                // 交互模式：提示用户检查网络连接
                console.log('连接已断开，请检查网络连接');
                authFailed = true;
                startClient();
            } else {
                // 服务模式：退出进程，让 systemd 自动重启
                console.log('[服务模式] 进程退出，等待 systemd 自动重启...');
                process.exit(1);
            }
            return;
        }
...
function executeOpenClawCommand(command, ws, sessionId, username) {
    ...
    const sessionState = {
        ...
        username: username || 'unknown' // 保存用户名，用于用户级别的任务中止
    };
...
function executeHermesCommand(command, ws, sessionId, username) {
    ...
    const spawnArgs = ['chat', '-q', finalCommand];
