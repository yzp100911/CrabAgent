...
let pollFailCount = 0; // 连续轮询失败计数（防抖用）
const POLL_FAIL_THRESHOLD = 3; // 连续失败 3 次才标记为空闲
...
async function pollCclawStatus() {
    try {
        ...
        // 本地 API 不可达，需要连续失败多次才标记为空闲（防止单次网络抖动误报）
        pollFailCount++;
        if (currentState.executing !== false && pollFailCount >= POLL_FAIL_THRESHOLD) {
            console.log(`[状态变更] cclaw 连接断开（连续 ${pollFailCount} 次失败），标记为空闲`);
        ...
        } else if (currentState.executing !== false) {
            console.log(`[监控] cclaw 连接失败 (${pollFailCount}/${POLL_FAIL_THRESHOLD})，暂不改变执行状态`);
        }
    }
}
